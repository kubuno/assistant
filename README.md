...
- **Conversations, folders & organization** — pin, rename, archive, drag-and-drop reordering, and project folders to group related conversations. Every conversation is addressable by URL (`/jarvis/#conversation/<id>`), so direct links and the browser Back button just work.
...
**Native packages** are attached to each [GitHub Release](https://github.com/kubuno/jarvis/releases) (built by CI on every `v*` tag):

| Platform | Package |
|---|---|
| Debian / Ubuntu | `kubuno-jarvis_*.deb` |
| Fedora / RHEL / openSUSE | `kubuno-jarvis-*.rpm` |
| Windows | `kubuno-jarvis-setup-*-x64.exe` (NSIS installer) |
| macOS | `kubuno-jarvis-*.pkg` |
...
```bash
bash build_rpm.sh          # Fedora/RHEL/openSUSE → dist/kubuno-jarvis-*.rpm
bash build_windows.sh      # Windows (NSIS)       → dist/kubuno-jarvis-setup-*-x64.exe
bash build_macos.sh        # macOS (on a Mac)     → dist/kubuno-jarvis-*.pkg
```
